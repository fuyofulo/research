...
├── stack.md                    ← Ramp Stack (Jun 3 2026): AI close OS for accounting FIRMS — attacks BILL's accountant-channel moat
...
- **[stack.md](stack.md)** — **Ramp Stack (launched Jun 3, 2026)** — an **AI operating system for accounting FIRMS** (not Ramp's usual in-house-finance buyer). Agents run the monthly close end-to-end: reconcile, code transactions, post journal entries, build recurring schedules (depreciation/prepaid/deferred-rev), flux analysis. Mechanics = **Skills (firm-authored plain-English SOPs = the firm's ownable IP) + Coworkers (agents that execute) + routines (scheduled)**; human-in-the-loop, every action logged. GA now ("free through August"); connects QBO/Drive/Plaid (NetSuite/Intacct roadmap). Launched **1 day before the $750M/$44B Series F** and named in it. Sells into 4,500+ firms ("92 of top 100 CPA firms already have clients on Ramp"). **Strategic significance: (1) a direct AI-native assault on Bill.com's deepest moat — the accountant channel, running BILL's own playbook AI-first; (2) the vertical productization of Ramp's agent-with-Skills architecture (validates our AP-teardown build thesis); (3) Ramp moving UP the pipeline into RECORD & CLOSE.** Benchmark (Stack 65.8% vs ChatGPT 61.7%/Claude 58.3%/Gemini 54.3%) is a Ramp-run internal eval 🔴; "50–60% faster close" is marketing.
